...
request_handler_interface* api_handler_factory::create(std::string location, std::string request_url, user_profile profile)
{
	return new api_handler(location, root_file_path_.info_map["data_path"], request_url, profile);
}